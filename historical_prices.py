...
from train_RNN import run_test_harness

uri = 'https://www.alphavantage.co/query?'
plt.close("all")
today_date = datetime.datetime.today().date()
historical = {'startmonth': (today_date.month % 12) + 1, 'startyear': today_date.year - 2, 'endmonth': today_date.month - 1,
...
def get_historical_intraday_data(parameters, period, counter):
...
def main():
    ...
    if len(symbols) == 0 and argtwo == 'single':
        symbols.append(arg)
    for symbol in symbols:
        ...
        if not pathlib.Path(symbol_folder).exists():
            ...
            os.mkdir(symbol_folder + '/data')
        historical_data_csv = generic_indicatior_parameters['symbol'] + '/data' + '/historical_data.csv'
        exists = False
        if pathlib.Path(historical_data_csv).exists():
            ...
            if modified_date == today_date:
                ...
                historical_data_current = get_historical_intraday_data(
                    parameters=generic_indicatior_parameters, period=current_month, counter=counter).reset_index()
                ...
                historical_data.index = pd.to_datetime(historical_data.index)
                historical_data.to_csv(generic_indicatior_parameters['symbol'] + '/data' + '/historical_data.csv')
        if not exists:
            historical_data = get_historical_intraday_data(
                parameters=generic_indicatior_parameters, period=historical, counter=counter)
            historical_data.to_csv(generic_indicatior_parameters['symbol'] + '/data' + '/historical_data.csv')
        historical_data_csv = generic_indicatior_parameters['symbol'] + '/data' + '/extended_historical_data.csv'
        if pathlib.Path(historical_data_csv).exists():
            modified_date = datetime.datetime.fromtimestamp(pathlib.Path(historical_data_csv).stat().st_mtime).date()
            if modified_date != today_date:
                ...
                extended_historical_data = pd.read_csv(historical_data_csv, index_col=0)
            ...
            extended_historical_data.to_csv(symbol_folder + '/data' +  '/extended_historical_data.csv')
        if skipfigure != True:
            for i, col in enumerate(extended_historical_data.columns):
                ...
                plt.close()
        run_test_harness(3,generic_indicatior_parameters['symbol'] ,'extended_historical_data.csv')
